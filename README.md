...
- **Agents** — 每个会话一行：项目名、来源、状态 + 持续时长、最后一条动作摘要。点击跳回对应的桌面端（`claude://code` / `codex://threads` / `cursor://file`，DSH Desktop 无 scheme 则拉起 app），没有深链就退回 Finder 定位日志。Claude 走的是**聚焦**已有会话、不新建；那条路由被官方功能开关关掉时，Dew 会自动识别并退回「导入一次」或访达定位。
- **定时任务** — Claude Code `scheduled-tasks` 与 Codex `automations`，显示下次触发时间（cron / RRULE 本地求值）。
- **To-do** — 高优 / 普通 / 每日重复 三类，键盘优先，纯本地 JSON。悬停一行可改名或删除；**完成的项第二天自动清空**（每日重复则是取消勾选、留在原地）。
...
- 中 / 英文切换，透明度可调，位置自动记忆，菜单栏入口，不占 Dock，可设开机自启。
...
3. **第一次打开**需要绕过 Gatekeeper——Dew 目前是自签名、未经 Apple 公证，系统默认会拦：
   ...
   只需要做一次，之后正常双击即可。
4. 第一次打开后，在设置面板里打开**「开机自启」**，之后登录就会自动出现，不用再手动开。
5. 打开后**没有 Dock 图标**：桌面上会出现一条小横条（折叠态），菜单栏右侧多一个 💧 水滴图标。点横条展开面板，`Esc` 或右上角箭头收起，拖动横条可以挪位置。
